...
const Component = ({ error, registration }) => {
  return (
    <div className="d-flex flex-column min-vh-100">
      <div className="d-flex justify-content-center align-items-center bg-white login-container py-4" style={{ minHeight: 'calc(100vh - 56px)' }}>
        <Image />
        <form className="border-0 px-3">
          <h3 className="mb-3 text-success">Just take a step to unlock the world</h3>

          <div className="mb-3">
            <label className="form-label">Email Address</label>
            <input type="email" name="email" placeholder="Enter a valid email address" className={`form-control ${error.email ? "text-danger border-danger" : ''}`} />
            <small className="text-danger">{error.email}</small>
          </div>

          <div className="mb-3">
            <label className="form-label">Password</label>
            <input type="password" name="password" placeholder="Enter password" className={`form-control ${error.password ? "text-danger border-danger" : ''}`} />
            <small className="text-danger">{error.password}</small>
          </div>

          {registration && <div className="mb-3">
            <label className="form-label">Confirm Password</label>
            <input type="password" name="confirmpassword" placeholder="Confirm Password" className={`form-control ${error.confirmpassword ? "text-danger border-danger" : ''}`} />
